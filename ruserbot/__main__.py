...
from ruserbot import LOGS, bot
from ruserbot.Config import Config
from ruserbot.utils import load_module
    ...
    with open(name) as f:
        ...
        try:
            if shortname.replace(".py", "") not in Config.NO_LOAD:
                ...
                os.remove(Path(f"ruserbot/plugins/{shortname}.py"))
        except Exception as e:
            os.remove(Path(f"ruserbot/plugins/{shortname}.py"))
            LOGS.info(f"unable to load {shortname} because of error {e}")

LOGS.info("Yay your ruserbot is officially working.!!!")
LOGS.info(
    "Congratulation, now type .alive to see message if bot is live\
    \nIf you need assistance, head to https://t.me/gabut_ngabuburit"
)
