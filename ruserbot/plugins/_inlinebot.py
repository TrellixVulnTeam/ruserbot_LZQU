...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("**ruserbot") and event.query.user_id == bot.uid:
            buttons = [
                (
                    custom.Button.inline("Stats", data="stats"),
                    Button.url("Repo", "https://github.com/rimuru07/ruserbot"),
                )
            ]
        ...
        elif event.query.user_id == bot.uid and query.startswith("Ruserbot"):
            rev_text = query[::-1]
            ...
            builder = event.builder
            secret = os.path.join("./Ruserbot", "secrets.txt")
            try:
            ...
            result = builder.article(
                ...
                text=f"🔒 A whisper message to {rimuru}, Only he/she can open it.",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = "Please get your own Ruserbot, and don't use mine! Join @gabut_ngabuburit help"
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert += (
                "Use .unload {} to remove this plugin ©Ruserbot".format(plugin_name)
            )
            try:
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
            except BaseException:
