...
async def cmd_list(event):
    ...
    if input_str == "text":
        string = "Total {count} commands found in {plugincount} plugins of ruserbot\n\n"
        catcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"**All commands of the ruserbot can be seen [here]({url})**"
    ...
    if input_str:
        ...
        if HELPTYPE is True:
            help_string = f"ruserbot Helper. Provided by {ALIVE_NAME} to reveal all the plugins\
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of ruserbot\n\n"
        )
        catcount = 0
        plugincount = 0
