...
async def download(event):
    ...
    if Config.GIT_REPO_NAME is None:
        await edit_delete(
            event, "`Please ADD Proper Github Repo Name of your ruserbot`", 5
        )
        return
...
async def git_commit(file_name, mone):
    ...
    file_name = "ruserbot/plugins/" + file_name
    if create_file:
        print(file_name)
        try:
            repo.create_file(
            ...
            ccess = ccess.strip()
            await mone.edit(
                f"`Commited On Your Github Repo`\n\n[Your PLUGINS](https://github.com/{ccess}/tree/master/ruserbot/plugins/)"
            )
